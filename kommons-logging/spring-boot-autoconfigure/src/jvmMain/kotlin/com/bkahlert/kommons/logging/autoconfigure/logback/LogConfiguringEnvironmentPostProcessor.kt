...
import com.bkahlert.kommons.autoconfigure.logback.Banners.turnOffBanner
import com.bkahlert.kommons.autoconfigure.logback.Banners.useSpringCloudBanner
import com.bkahlert.kommons.autoconfigure.logback.LogbackAutoConfiguration.Companion.LOGGING_CONFIG_RESOURCE
import com.bkahlert.kommons.autoconfigure.propertysource.YamlPropertySource
        ...
        if (SpringCloudDetection.isBootstrapApplicationContext(environment)) {
            ...
            StatusViaSLF4JLoggerFactory.addInfo(
                MessageFormat.format(
                    "Applying {0} to {1}@{2}", AnsiOutput.Enabled.ALWAYS,
                    application.javaClass.simpleName, application.hashCode()
                ), LogConfiguringEnvironmentPostProcessor::class.java.name
            )
