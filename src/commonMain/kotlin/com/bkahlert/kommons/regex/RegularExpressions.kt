...
import com.bkahlert.kommons.text.toCodePointList
...
public object RegularExpressions {
    ...
    public val versionRegex: Regex =
        Regex("(?<major>\\d+)\\.(?<minor>\\d+)\\.(?<patch>\\d+)") // kotlin.ByteArray.() -> com.bkahlert.kommons.math.BigInteger /* = java.math.BigInteger */
 ...
 *
 * Important: In Kotlin 1.0 group names are not supported but in order to still work [groupNames] need to contain all used group names in the order as their appear in the regular expression (e.g. `listOf("a", "b")`).
 */
