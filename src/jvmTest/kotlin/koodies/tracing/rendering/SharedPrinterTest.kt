...
import koodies.runtime.completableFuture
...
class SharedPrinterTest {
    ...
    @Test
    fun TestSpanScope.`should handle concurrent access`() {
        ...
        val result = listOf("foo", "bar", "baz").map { name ->
            executor.completableFuture {
                ...
                runSpanning(name, renderer = { BlockRenderer(Settings(decorationFormatter = color, contentFormatter = color, printer = sharedPrinter)) }) {
        ...
        runSpanning("main", renderer = { BlockRenderer(Settings(decorationFormatter = color, contentFormatter = color, printer = sharedPrinter)) }) {
            (0..5).forEach { log("${currentThread.name}: shared $it") }
            with(sharedPrinter) {
                runExclusive {
                    (10..15).forEach { log("exclusive $it") }
                    runSpanning("exclusive child") {
