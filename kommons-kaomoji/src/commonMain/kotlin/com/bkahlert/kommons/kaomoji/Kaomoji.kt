...
public data class Kaomoji(
    ...
    private constructor(
        ...
        leftArm.second.until(
            leftArm.second + leftEye.second
        ),
        (leftArm.second + leftEye.second).until(
            leftArm.second + leftEye.second + mouth.second
        ),
        (leftArm.second + leftEye.second + mouth.second).until(
            leftArm.second + leftEye.second + mouth.second + rightEye.second
        ),
        (leftArm.second + leftEye.second + mouth.second + rightEye.second).until(
            leftArm.second + leftEye.second + mouth.second + rightEye.second + rightArm.second
        ),
        (leftArm.second + leftEye.second + mouth.second + rightEye.second + rightArm.second).until(
            leftArm.second + leftEye.second + mouth.second + rightEye.second + rightArm.second + accessory.second
        )
    )
    ...
    /** Left arm of the [Kaomoji] or empty string if there's none. */
    public val leftArm: String get() = if (leftArmRange.isEmpty()) String.EMPTY else text.substring(leftArmRange)

    /** Left eye of the [Kaomoji] or empty string if there's none. */
    public val leftEye: String get() = if (leftEyeRange.isEmpty()) String.EMPTY else text.substring(leftEyeRange)

    /** Mouth of the [Kaomoji] or empty string if there's none. */
    public val mouth: String get() = if (mouthRange.isEmpty()) String.EMPTY else text.substring(mouthRange)

    /** Right eye of the [Kaomoji] or empty string if there's none. */
    public val rightEye: String get() = if (rightEyeRange.isEmpty()) String.EMPTY else text.substring(rightEyeRange)

    /** Right arm of the [Kaomoji] or empty string if there's none. */
    public val rightArm: String get() = if (rightArmRange.isEmpty()) String.EMPTY else text.substring(rightArmRange)

    /** Accessory of the [Kaomoji] or empty string if there's none. */
    public val accessory: String get() = if (accessoryRange.isEmpty()) String.EMPTY else text.substring(accessoryRange)
    ...
    public fun thinking(text: String? = null): String {
        val lines = text?.takeUnlessEmpty()?.lines() ?: emptyList()
        return when (lines.size) {
            ...
            2 -> {
                val max = lines.maxOf { it.columns }
                """
                    $blank       ⎛ ${lines.first().padEndColumns(max)} ⎞
                    $blank  ̣ ˱ ❨ ⎝ ${lines.last().padEndColumns(max)} ⎠
                    $this
                """.trimIndent()
            }
                ...
                val max = lines.maxOf { it.columns }
                "$blank       ⎛ ${lines.first().padEndColumns(max)} ⎞$LF" +
                    lines.drop(1).dropLast(1).joinToString("") { "$blank       ⎜ ${it.padEndColumns(max)} ⎟$LF" } +
                    "$blank  ̣ ˱ ❨ ⎝ ${lines.last().padEndColumns(max)} ⎠$LF" +
                    this
            }
        }
    }
    ...
    public companion object : Parser<Kaomoji>, Set<Kaomoji> {
        ...
        /**
         * Parses a [Kaomoji] from the given [text].
         */
        public override fun parse(text: CharSequence): Kaomoji {
            val string = text.toString()
            val parts = string.asCodePointSequence().toMutableList()
            ...
            return when (parts.size) {
                0, 1, 2 -> throw IllegalArgumentException("Kaomoji cannot be parsed. Only found parts: $parts")
                3 -> Kaomoji(
                    string,
                ...
                5 -> Kaomoji(
                    ...
                    rightEyeRange = ranges[3],
                    rightArmRange = ranges[4],
                    accessoryRange = ranges[5].first until text.length
                )
            }
        }
        ...
        /**
         * Selection of that's it [Kaomoji]
         */
        ...
        private val All: Set<Kaomoji> by lazy {
            setOf(
                Angry,
                Babies, BadMood, Bears, Begging, Blushing,
                Cats, Celebrities, Chasing, Confused, Crying, Cute,
                Dancing, Depressed, Devils, Disappointed, Dog, Drooling,
                Eating, Evil, Excited,
                FallingDown, Feces, Feminine, Fish, Fishing, Flower, Funny,
                Geek, Glasses, Greeting, Grinning, Gross,
                Happy, Helpless, Heroes, Hide, Hugging,
                Kissing,
                Laughing, LennyFace, Love,
                MakeUpMyMind, MiddleFinger, Money, Monkey, Musical,
                Nervious,
                PeaceSign, Pointing, Proud, Punching,
                Rabbits, Rain, RogerThat, RollOver, Running,
                Sad, Salute, Scared, Screaming, Sheep, Shocked, Shrugging, Shy, Sleeping,
                Smiling, Smoking, Sparkling, Spinning, StereoTypes, Surprised, Sweating,
                TableFlipping, TakeABow, ThatsIt, ThumbsUp, Tired, Trembling, TryMyBest, TV,
                Eyes, Upset,
                Vomitting,
                Weapons, Weird, Whales, Why, Winking, Wizards, Writing,
            ).flatten().toSet()
        }

        override val size: Int get() = All.size
        override fun isEmpty(): Boolean = All.isEmpty()
        override fun iterator(): Iterator<Kaomoji> = All.iterator()
        override fun containsAll(elements: Collection<Kaomoji>): Boolean = All.containsAll(elements)
        override fun contains(element: Kaomoji): Boolean = All.contains(element)
    }
}
