...
import com.bkahlert.kommons_deprecated.builder.Init
import com.bkahlert.kommons_deprecated.docker.MountOptionContext.Type.tmpfs
import com.bkahlert.kommons_deprecated.docker.MountOptionContext.Type.volume
...
class MountOptionsTest {
    ...
    inner class MapToHostPath {
        ...
        @Test
        fun `should map on single match`(simpleId: SimpleId) = withTempDir(simpleId) {
            expectThat(mountOptions).get { mapToHostPath("/mount/host2".asContainerPath()) }
                .isEqualTo(resolve("host/root2"))
        }

        @Test
        fun `should map first on multiple matches`(simpleId: SimpleId) = withTempDir(simpleId) {
            expectThat(mountOptions).get { mapToHostPath("/mount/host1/root3".asContainerPath()) }
                .isEqualTo(resolve("host/root1/root3"))
        }

        @Test
        fun `should throw on no match`(simpleId: SimpleId) = withTempDir(simpleId) {
